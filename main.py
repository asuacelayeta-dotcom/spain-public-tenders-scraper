...
Límites por defecto: max_results=2000, max_pages=60 (configurables por input)
...
def parse_entry(entry: etree._Element, ns: dict[str, str]) -> Optional[dict[str, Any]]:
    ...
    try:
        ...
        _nuts        = first_text(entry, [
            ".//cac:RealizedLocation/cbc:CountrySubentityCode/text()",
            ".//cac:RealizedLocation//cbc:CountrySubentityCode/text()",
            ".//cbc:CountrySubentityCode/text()",
        ], ns)
        ...
        return {
            ...
            "cpv":                        _cpv_codigo,
            "codigo_nuts":                _nuts,
            "presupuesto_sin_iva":        _presupuesto or None,
        }
    except Exception as e:
...
def apply_budget_filter(records: list[dict], minimo: Optional[float] = None) -> list[dict]:
    limite = PRESUPUESTO_MIN if minimo is None else float(minimo)
    if limite <= 0:
        return records
    return [
        r for r in records
        if r.get("_presupuesto", 0.0) == 0.0 or r.get("_presupuesto", 0.0) >= limite
    ]
...
def filter_by_cpv(records: list[dict], cpv: str) -> list[dict]:
    """Filtra por prefijo de CPV tal cual lo pide el usuario.

    No se recortan ceros: el CPV "90" debe filtrar la division 90
    (limpieza y residuos), nunca todo lo que empiece por "9".
    """
    prefix = (cpv or "").strip()
    if not prefix:
        return records
...
def filter_by_nuts(records: list[dict], nuts: str) -> list[dict]:
    """Filtra por codigo NUTS del lugar de ejecucion. Bizkaia = ES213."""
    prefix = (nuts or "").strip().upper()
    if not prefix:
        return records
    return [r for r in records if (r.get("_nuts") or "").upper().startswith(prefix)]
...
def motor_decision(records, keyword, tipo_contrato, cpv, nuts="", presupuesto_min=None):
    base = apply_exclusions(records)
    base = apply_budget_filter(base, presupuesto_min)
    base = filter_by_nuts(base, nuts)
    ...
    if nuts:
        filtros_secundarios.append(f"nuts='{nuts}'")
...
async def main() -> None:
    async with Actor:
        ...
        # Opcionales (si no estan en el INPUT_SCHEMA, se usan los valores por defecto)
        nuts            = normalizar(input_data.get("nuts")) or ""
        max_pages       = int(input_data.get("max_pages") or MAX_PAGES)
        max_results     = int(input_data.get("max_results") or MAX_RESULTS)
        presupuesto_min = input_data.get("presupuesto_min")
        presupuesto_min = float(presupuesto_min) if presupuesto_min is not None else PRESUPUESTO_MIN
        ...
        Actor.log.info(f"NUTS:             {nuts or '(toda España)'}")
        Actor.log.info(f"Presupuesto min:  {presupuesto_min:.0f} €")
        Actor.log.info(f"Max results:      {max_results}")
        Actor.log.info(f"Max páginas:      {max_pages}")
        Actor.log.info("=" * 60)
        ...
        resultados, filtro_primario, filtros_secundarios, dic_usado = motor_decision(
            ...
            nuts=nuts, presupuesto_min=presupuesto_min
        )
